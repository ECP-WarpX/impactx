        ...
        std::unordered_map<std::string, amrex::ParticleReal> reduced_beam_characteristics(ImpactXParticleContainer const & pc) {
            ...
            amrex::ParticleReal,amrex::ParticleReal,amrex::ParticleReal,amrex::ParticleReal,amrex::ParticleReal>>(
                            ...
                            amrex::ParticleReal,amrex::ParticleReal>
                    {
                        ...
                        const amrex::ParticleReal p_ux = ptd.m_rdata[RealSoA::ux][i];
                        const amrex::ParticleReal p_uy = ptd.m_rdata[RealSoA::uy][i];
                        ...
                        const amrex::ParticleReal p_ux_mean = p_ux*p_w;
                        const amrex::ParticleReal p_uy_mean = p_uy*p_w;
            ...
            amrex::ParticleReal ux_mean = values_per_rank_1st.at(4) /= w_sum;
            amrex::ParticleReal uy_mean = values_per_rank_1st.at(5) /= w_sum;
            ...
            auto r2 = amrex::ParticleReduce<amrex::ReduceData<amrex::ParticleReal,amrex::ParticleReal,
                                    ...
                                    amrex::ParticleReal,amrex::ParticleReal,amrex::ParticleReal,amrex::ParticleReal>
                    {
                        ...
                        const amrex::ParticleReal p_ux_ms = (p_ux-ux_mean)*(p_ux-ux_mean)*p_w;
                        const amrex::ParticleReal p_uy_ms = (p_uy-uy_mean)*(p_uy-uy_mean)*p_w;
                        const amrex::ParticleReal p_ut_ms = (p_ut-ut_mean)*(p_ut-ut_mean)*p_w;

                        const amrex::ParticleReal p_xux = (p_x-x_mean)*(p_ux-ux_mean)*p_w;
                        const amrex::ParticleReal p_yuy = (p_y-y_mean)*(p_uy-uy_mean)*p_w;
            ...
            amrex::ParticleReal ux_ms  = values_per_rank_2nd.at(3) /= w_sum;
            amrex::ParticleReal uy_ms  = values_per_rank_2nd.at(4) /= w_sum;
            amrex::ParticleReal ut_ms  = values_per_rank_2nd.at(5) /= w_sum;
            amrex::ParticleReal xux    = values_per_rank_2nd.at(6) /= w_sum;
            amrex::ParticleReal yuy    = values_per_rank_2nd.at(7) /= w_sum;
            ...
            amrex::ParticleReal sig_px = std::sqrt(ux_ms);
            amrex::ParticleReal sig_py = std::sqrt(uy_ms);
            amrex::ParticleReal sig_pt = std::sqrt(ut_ms);
            // RMS emittances
            amrex::ParticleReal emittance_x = std::sqrt(x_ms*ux_ms-xux*xux);
            amrex::ParticleReal emittance_y = std::sqrt(y_ms*uy_ms-yuy*yuy);
            ...
            amrex::ParticleReal alpha_x = - xux / emittance_x;
            amrex::ParticleReal alpha_y = - yuy / emittance_y;
            ...
            data["sig_y"] = sig_y;
            data["sig_t"] = sig_t;
            data["ux_mean"] = ux_mean;
            data["uy_mean"] = uy_mean;
