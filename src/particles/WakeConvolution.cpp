...
double kappa = (2 * rc * m_e * std::pow(c, 2)) / std::pow(3, 1.0/3.0) / std::pow(10.35, 2.0/3.0); //Assumes R = 10.35
...
void Convolve_FFT(const std::vector<double>& beam_profile, const std::vector<double>& wake_func, double delta_t, std::vector<double>& result)
{
    ...
    int n = beam_profile.size() + wake_func.size() - 1; //Output size is n = 2*N - 1, where N = size of signals 1,2
