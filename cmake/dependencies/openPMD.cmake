function(find_openpmd)
    if(ImpactX_openpmd_src)
        ...
        message(STATUS "openPMD-api source path: ${ImpactX_openpmd_src}")
    elseif(ImpactX_openpmd_internal)
        message(STATUS "Downloading openPMD-api ...")
        message(STATUS "openPMD-api repository: ${ImpactX_openpmd_repo} (${ImpactX_openpmd_branch})")
        include(FetchContent)
    endif()
    if(ImpactX_openpmd_internal OR ImpactX_openpmd_src)
        set(CMAKE_POLICY_DEFAULT_CMP0077 NEW)

        # see https://openpmd-api.readthedocs.io/en/0.14.1/dev/buildoptions.html
        set(openPMD_USE_MPI         ${ImpactX_MPI}  CACHE INTERNAL "")
        ...
        if(ImpactX_openpmd_src)
            add_subdirectory(${ImpactX_openpmd_src} _deps/localopenpmd-build/)
        else()
            FetchContent_Declare(fetchedopenpmd
                GIT_REPOSITORY ${ImpactX_openpmd_repo}
                GIT_TAG        ${ImpactX_openpmd_branch}
                BUILD_IN_SOURCE 0
            )
        ...
        # openPMD options not relevant to ImpactX users
