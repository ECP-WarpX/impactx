...
def to_t(ref_pz, ref_pt, data_arr_s, ref_z=None, coord_type=TCoords.REF): #x, y, t, dpx, dpy, dpt):
    ...
    if type(data_arr_s) is pd.core.frame.DataFrame:
        coordinate_columns = ['position_x','position_y','position_t', 'momentum_x', 'momentum_y', 'momentum_t']
        assert all(val in data_arr_s.columns for val in coordinate_columns), f"data_arr_s must have columns {' '.join(coordinate_columns)}"
        ...
        raise Exception(f"Incompatible input type {type(data_arr_s)} for data_arr_s, must be pandas DataFrame or Nx6 array-like object")
    x += ref_pz*dpx*t/(ref_pt+ref_pz*dpt)
    y += ref_pz*dpy*t/(ref_pt+ref_pz*dpt)
    pz = np.sqrt(-1 + (ref_pt + ref_pz*dpt)**2-(ref_pz*dpx)**2-(ref_pz*dpy)**2)
    ...
    if coord_type is TCoords.REF:
        ...
        assert ref_z is not None, "Reference particle z coordinate is required to transform to global coordinates"
...
def plot_beam_df(beam_at_step,axT,
                 unit=1e6,
                 unit_z=1e3,
                 unit_label='$\mu$m',
                 unit_z_label='mm',
...
impactx_surrogate_reduced_diags = read_time_series("diags/reduced_beam_characteristics.*")
ref_gamma = np.sqrt(1+impactx_surrogate_reduced_diags['ref_beta_gamma']**2)
beam_gamma = ref_gamma+impactx_surrogate_reduced_diags['pt_mean']*impactx_surrogate_reduced_diags['ref_beta_gamma']
...
ax = axT[0][0]
scale = 1e6
ax.plot(impactx_surrogate_reduced_diags['s'][ix_slice],
...
ax.plot(impactx_surrogate_reduced_diags['s'][ix_slice],
         (ref_gamma[ix_slice]- impactx_surrogate_reduced_diags['pt_mean'][ix_slice])*scale,
         'go')
ax.set_xlabel('s [m]')
...
ax = axT[1][1]
scale = 1e3
ax.semilogy(impactx_surrogate_reduced_diags['s'][ix_slice],
...
######## plot phase spaces ###########
beam_impactx_surrogate_series = io.Series("diags/openPMD/monitor.bp", io.Access.read_only)
...
to_t(ref_u, ref_part_step['pt'], beam_at_step, ref_z=ref_part_step['z'],coord_type=TCoords.GLOBAL)
