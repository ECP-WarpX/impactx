...
class surrogate_model:
    ...
    def __init__(self, model_file,device=None):
        ...
        self.source_means = torch.tensor(model_dict["source_means"], device=self.device, dtype=torch.float64)
        self.target_means = torch.tensor(model_dict["target_means"], device=self.device, dtype=torch.float64)
        self.source_stds = torch.tensor(model_dict["source_stds"], device=self.device, dtype=torch.float64)
        self.target_stds = torch.tensor(model_dict["target_stds"], device=self.device, dtype=torch.float64)
        ...
        if 'n_hidden_layers' in model_dict.keys():
            ...
            if activation is Activation.PReLU:
                n_hidden_layers = int((len(model_dict['model_state_dict'].keys())-2)/3)
            else:
                n_hidden_layers = int(len(model_dict['model_state_dict'].keys())/2-1)
        
        self.neural_network = OneActNN(n_in=n_in, 
                                          n_out=n_out, 
