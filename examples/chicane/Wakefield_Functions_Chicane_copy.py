...
for i in range(len(beam_pdf) - 1):
    num_slopes.append((beam_pdf[i+1] - beam_pdf[i]) / (beam_initial_t_padded[i+1] - beam_initial_t_padded[i]))
...
convolved_signal_func = convolve_with_fft(func_3(beam_initial_t_padded), func_4(beam_initial_t_padded), delta_t)
convolved_signal_CSR = convolve_with_fft(num_slopes, W_L_CSR(beam_initial_t_padded, R), delta_t)
convolved_signal_CSR_nopad = convolve_with_fft(num_slopes_nopad, W_L_CSR(beam_initial_t, R), delta_t)
...
t_range_nopad = np.linspace(lower_bound_nopad, upper_bound_nopad, 2 * len(beam_initial_t) - 1)
...
convolved_signal_RF_x = convolve_with_fft(signal_resistive_wall_x, W_T_RF(beam_initial_t_padded, a, g, L), delta_t)
convolved_signal_RF_y = convolve_with_fft(signal_resistive_wall_y, W_T_RF(beam_initial_t_padded, a, g, L), delta_t)
convolved_signal_RF_z = convolve_with_fft(beam_initial_num_padded, W_L_RF(beam_initial_t_padded, a, g, L), delta_t)
