...
beam_final_num_padded = np.pad(beam_final_num, (pad_before, pad_after), 'constant') #param constant = 0
beam_final_charge_padded = np.pad(beam_final_charge, (pad_before, pad_after), 'constant')
...
for i in range(len(beam_final_charge_padded) - 1):
    charge_slopes.append((beam_final_charge_padded[i+1] - beam_final_charge_padded[i]) / (beam_final_t_padded[i+1] - beam_final_t_padded[i]))

num_slopes = []
for i in range(len(beam_pdf) - 1):
    num_slopes.append((beam_pdf[i+1] - beam_pdf[i]) / (beam_final_t_padded[i+1] - beam_final_t_padded[i]))
...
convolved_signal_CSR = convolve_with_fft(num_slopes, W_L_CSR(beam_final_t_padded, R), delta_t)
...
convolved_signal_RF_x = convolve_with_fft(signal_resistive_wall_x, W_T_RF(beam_final_t_padded, a, g, L), delta_t)
convolved_signal_RF_y = convolve_with_fft(signal_resistive_wall_y, W_T_RF(beam_final_t_padded, a, g, L), delta_t)
convolved_signal_RF_z = convolve_with_fft(beam_final_num_padded, W_L_RF(beam_final_t_padded, a, g, L), delta_t)
