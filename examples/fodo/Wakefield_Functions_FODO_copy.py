...
def W_T_RF(s, a, g, L):
    ...
    term = 1 - (1 + np.sqrt(np.abs(s) / s0)) * np.exp(
        -np.sqrt(np.abs(s) / s0)
    )  # new term
...
for i in range(len(beam_pdf) - 1):
    num_slopes.append(
        ...
        / (beam_initial_t_padded[i + 1] - beam_initial_t_padded[i])
    )  # denom changes w/n_bins - is constant per run
...
kaiser_window = np.kaiser(
    len(convolved_signal_CSR), 20
)  # Artifact goes away at beta = 50
...
plt.plot(
    normalized_t[:-1], convolved_signal_CSR_blackman * 1e9, label="Blackman Window"
)
plt.plot(
    normalized_t[:-1],
    convolved_signal_CSR_kaiser * 1e9,
    label="Kaiser Window, Beta = 20",
)
...
convolved_signal_RF_x = convolve_with_fft(
    signal_resistive_wall_x, W_T_RF(beam_initial_t_padded, a[0], g[0], L), delta_t
)
convolved_signal_RF_y = convolve_with_fft(
    signal_resistive_wall_y, W_T_RF(beam_initial_t_padded, a[0], g[0], L), delta_t
)
convolved_signal_RF_z = convolve_with_fft(
    beam_pdf, W_L_RF(beam_initial_t_padded, a[0], g[0], L), delta_t
)
...
for i, s in enumerate(beam_initial_t_padded):
    ...
    # Integrate over positions s'
    conv_num_int_T[i] = simps(
        shifted_W_T * (x * rho), dx=delta_t
    )  # beam_mean_x_padded for x
...
plt.plot(
    beam_initial_t_padded / sigma_t,
    conv_num_int_L * 1e-15,
    label="Convolution (Integration)",
)
...
plt.plot(
    beam_initial_t_padded / sigma_t,
    conv_num_int_T * 1e-12,
    label="Convolution (Numerical Integration)",
)
